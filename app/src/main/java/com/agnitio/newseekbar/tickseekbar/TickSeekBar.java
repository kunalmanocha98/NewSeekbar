package com.agnitio.newseekbar.tickseekbar;
...
import com.agnitio.newseekbar.R;
...
//        tickTexts
    ...
    private void initParams() {
        ...
        if (mThumbDrawable == null) {
            ...
            mThumbTouchRadius = mThumbRadius * 5.0f;
        } else {
...
//        initTickXAndTextXLocation();
    ...
    private Bitmap getDrawPressedBitmap(Drawable drawable, boolean isThumb) {
        if (drawable == null) {
            return null;
        }
        int width;
...
//            mPressedThumbBitmap = mThumbBitmap;
            mPressedThumbBitmap=getDrawPressedBitmap(mThumbDrawable,true);
...
//            mTextPaint.setTypeface(mTextsTypeface);
            mTextPaint.setTypeface(Typeface.create(mTextsTypeface, Typeface.BOLD));
    ...
    private void initTickTextsYLocation() {
        if (needDrawText()) {
            ...
            if (hasBelowText()) {//both text below seek bar
                ...
                mTickTextY = -24 + mPaddingTop + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + SizeUtils.dp2px(mContext, 3);
            }
...
//        Iterator iterator=Arrays.asList(mProgressArr).iterator();
        for (int i = 0; i < mProgressArr.length; i++) {
            if (mProgressArr[i] == mProgress) {
                index = i;
                break;
            }
        }
        counterposition = Integer.parseInt(getTickTextByPosition(index));
    ...
    public synchronized void setProgress(float progress) {
        lastProgress = mProgress;
        mProgress = MathUtils.clamp(progress, mMin, mMax);
//        adjust to the closest tick's progress
    ...
    public synchronized void setProgress(String progress){
        int indexvalue= Arrays.asList(mTickTextsArr).indexOf(progress);
        lastProgress = mProgress;
        mProgress = MathUtils.clamp(mProgressArr[indexvalue], mMin, mMax);
